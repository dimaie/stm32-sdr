...
static uint64_t si5351_pll_calc(Si5351*, enum si5351_pll, uint64_t,
		struct Si5351RegSet*, int32_t, uint8_t);
static uint64_t si5351_multisynth_calc(Si5351*, uint64_t, uint64_t,
		struct Si5351RegSet*);
static uint64_t si5351_multisynth67_calc(Si5351*, uint64_t, uint64_t,
		struct Si5351RegSet*);
static void si5351_update_sys_status(Si5351*, struct Si5351Status*);
static void si5351_update_int_status(Si5351*, struct Si5351IntStatus*);
static void si5351_ms_div(Si5351*, enum si5351_clock, uint8_t, uint8_t);
static uint8_t si5351_select_r_div(Si5351*, uint64_t*);
static uint8_t si5351_select_r_div_ms67(Si5351*, uint64_t*);
static uint8_t si5351_write_bulk(Si5351*, uint8_t, uint8_t, uint8_t*);
static uint8_t si5351_write(Si5351*, uint8_t, uint8_t);
static uint8_t si5351_read(Si5351*, uint8_t);

void si5351_create(Si5351 *self, uint8_t i2c_addr) {
	self->i2c_bus_addr = i2c_addr;
	self->xtal_freq[0] = SI5351_XTAL_FREQ;

	// Start by using XO ref osc as default for each PLL
	self->plla_ref_osc = SI5351_PLL_INPUT_XO;
	self->pllb_ref_osc = SI5351_PLL_INPUT_XO;
	self->clkin_div = SI5351_CLKIN_DIV_1;
}
 ...
 *
 * Returns a uint8_tean that indicates whether a device was found on the desired
 * I2C address.
 *
 */
uint8_t si5351_init(Si5351 *self, uint8_t xtal_load_c, uint32_t xo_freq,
		int32_t corr) {
	// Start I2C comms
//	Wire.begin();
...
//  reg_val = Wire.endTransmission();
	...
	if (reg_val == 0) {
		...
		do {
			status_reg = si5351_read(self, SI5351_DEVICE_STATUS);
		} while (status_reg >> 7 == 1);

		// Set crystal load capacitance
		si5351_write(self, SI5351_CRYSTAL_LOAD,
				(xtal_load_c & SI5351_CRYSTAL_LOAD_MASK) | 0b00010010);

		// Set up the XO and CLKIN reference frequencies
		if (xo_freq != 0) {
			si5351_set_ref_freq(self, xo_freq, SI5351_PLL_INPUT_XO);
			si5351_set_ref_freq(self, xo_freq, SI5351_PLL_INPUT_CLKIN); //Also CLKIN
		} else {
			si5351_set_ref_freq(self, SI5351_XTAL_FREQ, SI5351_PLL_INPUT_XO);
			si5351_set_ref_freq(self, SI5351_XTAL_FREQ, SI5351_PLL_INPUT_CLKIN); //Also CLKIN
		}

		// Set the frequency calibrations for the XO and CLKIN
		si5351_set_correction(self, corr, SI5351_PLL_INPUT_XO);
		si5351_set_correction(self, corr, SI5351_PLL_INPUT_CLKIN);
 ...
 *
 */
void si5351_reset(Si5351 *self) {
	// Initialize the CLK outputs according to flowchart in datasheet
	// First, turn them off
	si5351_write(self, 16, 0x80);
	si5351_write(self, 17, 0x80);
	si5351_write(self, 18, 0x80);
	si5351_write(self, 19, 0x80);
	si5351_write(self, 20, 0x80);
	si5351_write(self, 21, 0x80);
	si5351_write(self, 22, 0x80);
	si5351_write(self, 23, 0x80);

	// Turn the clocks back on...
	si5351_write(self, 16, 0x0c);
	si5351_write(self, 17, 0x0c);
	si5351_write(self, 18, 0x0c);
	si5351_write(self, 19, 0x0c);
	si5351_write(self, 20, 0x0c);
	si5351_write(self, 21, 0x0c);
	si5351_write(self, 22, 0x0c);
	si5351_write(self, 23, 0x0c);

	// Set PLLA and PLLB to 800 MHz for automatic tuning
	si5351_set_pll(self, SI5351_PLL_FIXED, SI5351_PLLA);
	si5351_set_pll(self, SI5351_PLL_FIXED, SI5351_PLLB);

	// Make PLL to CLK assignments for automatic tuning
	self->pll_assignment[0] = SI5351_PLLA;
	self->pll_assignment[1] = SI5351_PLLA;
	self->pll_assignment[2] = SI5351_PLLA;
	self->pll_assignment[3] = SI5351_PLLA;
	self->pll_assignment[4] = SI5351_PLLA;
	self->pll_assignment[5] = SI5351_PLLA;
	self->pll_assignment[6] = SI5351_PLLB;
	self->pll_assignment[7] = SI5351_PLLB;

	si5351_set_ms_source(self, SI5351_CLK0, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK1, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK2, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK3, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK4, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK5, SI5351_PLLA);
	si5351_set_ms_source(self, SI5351_CLK6, SI5351_PLLB);
	si5351_set_ms_source(self, SI5351_CLK7, SI5351_PLLB);

	// Reset the VCXO param
	si5351_write(self, SI5351_VXCO_PARAMETERS_LOW, 0);
	si5351_write(self, SI5351_VXCO_PARAMETERS_MID, 0);
	si5351_write(self, SI5351_VXCO_PARAMETERS_HIGH, 0);

	// Then reset the PLLs
	si5351_pll_reset(self, SI5351_PLLA);
	si5351_pll_reset(self, SI5351_PLLB);

	// Set initial frequencies
	uint8_t i;
	for (i = 0; i < 8; i++) {
		self->clk_freq[i] = 0;
		si5351_output_enable(self, (enum si5351_clock) i, 0);
		self->clk_first_set[i] = 0;
	}
}
 ...
 */
uint8_t si5351_set_freq(Si5351 *self, uint64_t freq, enum si5351_clock clk) {
	...
	if ((uint8_t) clk <= (uint8_t) SI5351_CLK5) {
		...
		if (freq > (SI5351_MULTISYNTH_SHARE_MAX * SI5351_FREQ_MULT)) {
			...
			for (i = 0; i < 6; i++) {
				if (self->clk_freq[i]
					...
					if (i != (uint8_t) clk
							&& self->pll_assignment[i]
									== self->pll_assignment[clk]) {
			...
			if (self->clk_first_set[(uint8_t) clk] == 0) {
				si5351_output_enable(self, clk, 1);
				self->clk_first_set[(uint8_t) clk] = 1;
			}

			// Set the freq in memory
			self->clk_freq[(uint8_t) clk] = freq;

			// Calculate the proper PLL frequency
			pll_freq = si5351_multisynth_calc(self, freq, 0, &ms_reg);

			// Set PLL
			si5351_set_pll(self, pll_freq, self->pll_assignment[clk]);

			// Recalculate params for other synths on same PLL
			for (i = 0; i < 6; i++) {
				if (self->clk_freq[i] != 0) {
					if (self->pll_assignment[i] == self->pll_assignment[clk]) {
						struct Si5351RegSet temp_reg;
						uint64_t temp_freq;

						// Select the proper R div value
						temp_freq = self->clk_freq[i];
						r_div = si5351_select_r_div(self, &temp_freq);

						si5351_multisynth_calc(self, temp_freq, pll_freq,
								&temp_reg);
						...
						si5351_set_ms(self, (enum si5351_clock) i, temp_reg,
			...
			// Reset the PLL
			si5351_pll_reset(self, self->pll_assignment[clk]);
		} else {
			self->clk_freq[(uint8_t) clk] = freq;
			...
			// Select the proper R div value
			r_div = si5351_select_r_div(self, &freq);

			// Calculate the synth parameters
			if (self->pll_assignment[clk] == SI5351_PLLA) {
				si5351_multisynth_calc(self, freq, self->plla_freq, &ms_reg);
			} else {
				si5351_multisynth_calc(self, freq, self->pllb_freq, &ms_reg);
			}

			// Set multisynth registers
			si5351_set_ms(self, clk, ms_reg, int_mode, r_div, div_by_4);

			// Reset the PLL
			//pll_reset(self->pll_assignment[clk]);
		}
		...
		if (clk == SI5351_CLK6) {
			if (self->clk_freq[7] != 0) {
				if (self->pllb_freq % freq == 0) {
					if ((self->pllb_freq / freq) % 2 != 0) {
						...
						// Select the proper R div value
						r_div = si5351_select_r_div_ms67(self, &freq);

						si5351_multisynth67_calc(self, freq, self->pllb_freq,
								&ms_reg);
					}
				} else {
				...
				pll_freq = si5351_multisynth67_calc(self, freq, 0, &ms_reg);
				//self->pllb_freq = pll_freq;
				si5351_set_pll(self, pll_freq, SI5351_PLLB);
			}
		} else {
			if (self->clk_freq[6] != 0) {
				if (self->pllb_freq % freq == 0) {
					if ((self->pllb_freq / freq) % 2 != 0) {
						// Not an even divide ratio, no bueno
						return 1;
 ...
 *   (use the si5351_clock enum)
 */
uint8_t si5351_set_freq_manual(Si5351 *self, uint64_t freq, uint64_t pll_freq,
	...
	// Calculate the synth parameters
	si5351_multisynth_calc(self, freq, pll_freq, &ms_reg);
 ...
 */
void si5351_set_pll(Si5351 *self, uint64_t pll_freq, enum si5351_pll target_pll) {
	struct Si5351RegSet pll_reg;

	if (target_pll == SI5351_PLLA) {
		si5351_pll_calc(self, SI5351_PLLA, pll_freq, &pll_reg,
				self->ref_correction[self->plla_ref_osc], 0);
	} else {
		si5351_pll_calc(self, SI5351_PLLB, pll_freq, &pll_reg,
				self->ref_correction[self->pllb_ref_osc], 0);
	}
	...
	if (target_pll == SI5351_PLLA) {
		si5351_write_bulk(self, SI5351_PLLA_PARAMETERS, i, params);
		self->plla_freq = pll_freq;
	} else if (target_pll == SI5351_PLLB) {
		si5351_write_bulk(self, SI5351_PLLB_PARAMETERS, i, params);
		self->pllb_freq = pll_freq;
	}
}
 ...
 *   Set to 1 to enable, 0 to disable
 */
void si5351_set_ms(Si5351 *self, enum si5351_clock clk,
		struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div,
		uint8_t div_by_4) {
	uint8_t params[20];
	uint8_t i = 0;
	uint8_t temp;
	...
	if ((uint8_t) clk <= (uint8_t) SI5351_CLK5) {
		...
		reg_val = si5351_read(self, (SI5351_CLK0_PARAMETERS + 2) + (clk * 8));
		reg_val &= ~(0x03);
	...
	switch (clk) {
	case SI5351_CLK0:
		si5351_write_bulk(self, SI5351_CLK0_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK1:
		si5351_write_bulk(self, SI5351_CLK1_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK2:
		si5351_write_bulk(self, SI5351_CLK2_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK3:
		si5351_write_bulk(self, SI5351_CLK3_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK4:
		si5351_write_bulk(self, SI5351_CLK4_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK5:
		si5351_write_bulk(self, SI5351_CLK5_PARAMETERS, i, params);
		si5351_set_int(self, clk, int_mode);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK6:
		si5351_write(self, SI5351_CLK6_PARAMETERS, temp);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	case SI5351_CLK7:
		si5351_write(self, SI5351_CLK7_PARAMETERS, temp);
		si5351_ms_div(self, clk, r_div, div_by_4);
		break;
	}
}
 ...
 */
void si5351_output_enable(Si5351 *self, enum si5351_clock clk, uint8_t enable) {
	uint8_t reg_val;

	reg_val = si5351_read(self, SI5351_OUTPUT_ENABLE_CTRL);
	...
	si5351_write(self, SI5351_OUTPUT_ENABLE_CTRL, reg_val);
}
 ...
 */
void si5351_drive_strength(Si5351 *self, enum si5351_clock clk,
		enum si5351_drive drive) {
	uint8_t reg_val;
	const uint8_t mask = 0x03;

	reg_val = si5351_read(self, SI5351_CLK0_CTRL + (uint8_t) clk);
	reg_val &= ~(mask);
	...
	si5351_write(self, SI5351_CLK0_CTRL + (uint8_t) clk, reg_val);
}
 ...
 */
void si5351_update_status(Si5351 *self) {
	si5351_update_sys_status(self, &(self->dev_status));
	si5351_update_int_status(self, &(self->dev_int_status));
}
 ...
 * crystal.
 */
void si5351_set_correction(Si5351 *self, int32_t corr,
		enum si5351_pll_input ref_osc) {
	self->ref_correction[(uint8_t) ref_osc] = corr;

	// Recalculate and set PLL freqs based on correction value
	si5351_set_pll(self, self->plla_freq, SI5351_PLLA);
	si5351_set_pll(self, self->pllb_freq, SI5351_PLLB);
}
 ...
 */
void si5351_set_phase(Si5351 *self, enum si5351_clock clk, uint8_t phase) {
	// Mask off the upper bit since it is reserved
	phase = phase & 0b01111111;

	si5351_write(self, SI5351_CLK0_PHASE_OFFSET + (uint8_t) clk, phase);
}
 ...
 * in RAM.
 */
int32_t si5351_get_correction(Si5351 *self, enum si5351_pll_input ref_osc) {
	return self->ref_correction[(uint8_t) ref_osc];
}
 ...
 */
void si5351_pll_reset(Si5351 *self, enum si5351_pll target_pll) {
	if (target_pll == SI5351_PLLA) {
		si5351_write(self, SI5351_PLL_RESET, SI5351_PLL_RESET_A);
	} else if (target_pll == SI5351_PLLB) {
		si5351_write(self, SI5351_PLL_RESET, SI5351_PLL_RESET_B);
	}
}
 ...
 */
void si5351_set_ms_source(Si5351 *self, enum si5351_clock clk,
		enum si5351_pll pll) {
	uint8_t reg_val;
	...
	self->pll_assignment[(uint8_t) clk] = pll;
}
 ...
 */
void si5351_set_int(Si5351 *self, enum si5351_clock clk, uint8_t enable) {
	uint8_t reg_val;
	reg_val = si5351_read(self, SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 * saving feature).
 */
void si5351_set_clock_pwr(Si5351 *self, enum si5351_clock clk, uint8_t pwr) {
	uint8_t reg_val; //, reg;
	reg_val = si5351_read(self, SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 */
void si5351_set_clock_invert(Si5351 *self, enum si5351_clock clk, uint8_t inv) {
	uint8_t reg_val;
	reg_val = si5351_read(self, SI5351_CLK0_CTRL + (uint8_t) clk);
 ...
 * the clock output.
 */
void si5351_set_clock_source(Si5351 *self, enum si5351_clock clk,
 ...
 */
void si5351_set_clock_disable(Si5351 *self, enum si5351_clock clk,
 ...
 */
void si5351_set_clock_fanout(Si5351 *self, enum si5351_clock_fanout fanout,
		uint8_t enable) {
	uint8_t reg_val;
	reg_val = si5351_read(self, SI5351_FANOUT_ENABLE);
	...
	si5351_write(self, SI5351_FANOUT_ENABLE, reg_val);
}
 ...
 */
void si5351_set_pll_input(Si5351 *self, enum si5351_pll pll,
		enum si5351_pll_input input) {
	uint8_t reg_val;
	reg_val = si5351_read(self, SI5351_PLL_INPUT_SOURCE);
	...
	case SI5351_PLLA:
		if (input == SI5351_PLL_INPUT_CLKIN) {
			...
			reg_val |= self->clkin_div;
			self->plla_ref_osc = SI5351_PLL_INPUT_CLKIN;
		} else {
			reg_val &= ~(SI5351_PLLA_SOURCE);
			self->plla_ref_osc = SI5351_PLL_INPUT_XO;
		}
	...
	case SI5351_PLLB:
		if (input == SI5351_PLL_INPUT_CLKIN) {
			...
			reg_val |= self->clkin_div;
			self->pllb_ref_osc = SI5351_PLL_INPUT_CLKIN;
		} else {
			reg_val &= ~(SI5351_PLLB_SOURCE);
			self->pllb_ref_osc = SI5351_PLL_INPUT_XO;
		}
	...
	si5351_write(self, SI5351_PLL_INPUT_SOURCE, reg_val);
 ...
 */
void si5351_set_vcxo(Si5351 *self, uint64_t pll_freq, uint8_t ppm) {
	struct Si5351RegSet pll_reg;
	uint64_t vcxo_param;
	...
	// Set PLLB params
	vcxo_param = si5351_pll_calc(self, SI5351_PLLB, pll_freq, &pll_reg,
			self->ref_correction[self->pllb_ref_osc], 1);
	...
	si5351_write(self, SI5351_VXCO_PARAMETERS_LOW, temp);

	temp = (uint8_t) ((vcxo_param >> 8) & 0xFF);
	si5351_write(self, SI5351_VXCO_PARAMETERS_MID, temp);

	temp = (uint8_t) ((vcxo_param >> 16) & 0x3F);
	si5351_write(self, SI5351_VXCO_PARAMETERS_HIGH, temp);
}
 ...
 */
void si5351_set_ref_freq(Si5351 *self, uint32_t ref_freq,
		enum si5351_pll_input ref_osc) {
	// uint8_t reg_val;
...
uint8_t si5351_write_bulk(Si5351* self, uint8_t addr, uint8_t bytes, uint8_t *data) {
    if (HAL_I2C_Mem_Write(&hi2c1, self->i2c_bus_addr << 1, addr,
                          I2C_MEMADD_SIZE_8BIT, data, bytes, 100) != HAL_OK) {
        return 0xFF; // error
    }
    return 0;
}

uint8_t si5351_write(Si5351 *self, uint8_t addr, uint8_t data) {
    if (HAL_I2C_Mem_Write(&hi2c1, self->i2c_bus_addr << 1, addr,
...
uint8_t si5351_read(Si5351 *self, uint8_t addr) {
    uint8_t reg_val = 0;
    if (HAL_I2C_Mem_Read(&hi2c1, self->i2c_bus_addr << 1, addr,
                         I2C_MEMADD_SIZE_8BIT, &reg_val, 1, 100) != HAL_OK) {
        return 0xFF; // error
    }
    return reg_val;
}
...
uint64_t si5351_pll_calc(Si5351 *self, enum si5351_pll pll, uint64_t freq,
		struct Si5351RegSet *reg, int32_t correction, uint8_t vcxo) {
	uint64_t ref_freq;
	if (pll == SI5351_PLLA) {
		ref_freq = self->xtal_freq[(uint8_t) self->plla_ref_osc] * SI5351_FREQ_MULT;
	} else {
		ref_freq = self->xtal_freq[(uint8_t) self->pllb_ref_osc] * SI5351_FREQ_MULT;
	}
...
uint64_t si5351_multisynth_calc(Si5351 *self, uint64_t freq, uint64_t pll_freq,
...
uint64_t si5351_multisynth67_calc(Si5351 *self, uint64_t freq,
		uint64_t pll_freq, struct Si5351RegSet *reg) {
	//uint8_t p1;
	// uint8_t ret_val = 0;
	uint32_t a;
	uint64_t lltmp;
...
void si5351_update_sys_status(Si5351 *self, struct Si5351Status *status) {
	uint8_t reg_val = 0;

	reg_val = si5351_read(self, SI5351_DEVICE_STATUS);
...
void si5351_update_int_status(Si5351 *self, struct Si5351IntStatus *int_status) {
	uint8_t reg_val = 0;

	reg_val = si5351_read(self, SI5351_INTERRUPT_STATUS);
...
void si5351_ms_div(Si5351 *self, enum si5351_clock clk, uint8_t r_div,
		uint8_t div_by_4) {
	uint8_t reg_val = 0;
	uint8_t reg_addr = 0;
	...
	reg_val = si5351_read(self, reg_addr);
	...
	si5351_write(self, reg_addr, reg_val);
}

uint8_t si5351_select_r_div(Si5351 *self, uint64_t *freq) {
...
uint8_t si5351_select_r_div_ms67(Si5351 *self, uint64_t *freq) {
	uint8_t r_div = SI5351_OUTPUT_CLK_DIV_1;
